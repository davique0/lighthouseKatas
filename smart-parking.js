...
const whereCanIPark = function (spots, vehicle) {
  ...
  let typeOfVehicle = vehicle === 'regular' ? 'R' : vehicle === 'small' ? 'S' : 'M';
  let foundPrk = true;
  for(let y = 0; y < spots.length; y++) {
    if (foundPrk){
      for(let x = 0; x < spots[y].length; x++) {
        let parkSpace = spots[y][x]
        if (typeOfVehicle === 'M' && (parkSpace === 'M' || parkSpace === 'S' || parkSpace === 'R')) {
          spaceAvailable = [x, y];
          foundPrk = false;
          break;
        } else if (typeOfVehicle === 'S' && (parkSpace === 'S' || parkSpace === 'R')) {
          spaceAvailable = [x, y];
          foundPrk = false;
          break;
        } else if (typeOfVehicle === 'R' && parkSpace === 'R') {
          spaceAvailable = [x, y];
          foundPrk = false;
          break;
        }
